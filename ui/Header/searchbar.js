      ...
      <div className="relative items-center pl-1 flex w-full lg:max-w-68 sm:pr-2 sm:ml-0">
        <div className="container relative left-0 z-50 flex w-3/4 h-auto h-full">
          <div className="relative flex items-center w-full lg:w-70 h-full group">
            <SearchIcon className="absolute left-0 z-20 hidden w-5 h-5 ml-3 text-gray-500 pointer-events-none  group-hover:text-gray-400 sm:block" />
            ...
            <input
              ...
              placeholder="Search"
            />
            <UploadIcon className="absolute right-0 hidden w-5 h-5 mr-2 text-xs text-gray-400 cursor-pointer  md:block" />
          </div>
        </div>
      </div>
    </div>
